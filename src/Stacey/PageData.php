...
class PageData
{
    ...
    public static function extract_closest_siblings($siblings, $file_path)
    {
        $neighbors = array();
        # flip keys/values
    ...
    public static function get_parent($file_path, $url)
    {
    ...
    public static function get_parents($file_path, $url)
    {
        # split file path by slashes
        $split_path = explode('/', $file_path);
        $parents = array();
    ...
    public static function get_thumbnail($file_path)
    {
    ...
    public static function get_index($siblings, $file_path)
    {
        $count = 0;
    ...
    public static function is_current($base_url, $permalink)
    {
        $base_path = preg_replace('/^[^\/]+/', '', $base_url);

        return ($permalink == 'index')
            ? '/' == $_SERVER['REQUEST_URI']
            : $base_path.'/'.$permalink == $_SERVER['REQUEST_URI'];
    }

    public static function get_file_types($file_path)
    {
        $file_types = array();
    ...
    public static function create_vars($page)
    {
        # page.file_path
        ...
        # page.stacey_version
        $page->stacey_version = Application::$version;
        # page.domain_name
    ...
    public static function create_collections($page)
    {
        # page.root
    ...
    public static function create_asset_collections($page)
    {
        # page.files
    ...
    public static function get_shared_data()
    {
        ...
        if (file_exists($shared_file_path)) {
            return self::$shared = \Symfony\Component\Yaml\Yaml::parse(file_get_contents($shared_file_path));
        } else {
            return array();
        }
    }

    public static function preparse_text($text)
    {
    ...
    public static function create_textfile_vars($page, $content = false)
    {
        # store contents of content file (if it exists, otherwise, pass back an empty string)
        if ($content) {
            $vars = \Symfony\Component\Yaml\Yaml::parse(file_get_contents($content));
        } else {
            ...
            $vars = \Symfony\Component\Yaml\Yaml::parse($content);
        }
    ...
    public static function parse_vars($vars, $markdown_compatible, $relative_path)
    {
        foreach ($vars as $key => $value) {
            ...
            if (!is_string($value)) {
                $vars[$key] = $value;
            } elseif ($markdown_compatible && strpos($value, "\n") !== false) {
    ...
    public static function html_to_xhtml(&$value)
    {
    ...
    public static function clean_json($value)
    {
    ...
    public static function create($page, $content = false)
    {
        ...
        } elseif (preg_match('/\.(js|json)$/', $current_page_template_file)) {
