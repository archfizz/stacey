...
class Application
{
  static $version = '4.0.0-dev';

  public $route;

  public function __construct(array $get = array())
  {
    # sometimes when PHP release a new version, they do silly things - this public function is here to fix them
    $this->php_fixes();
  ...
  public function set_content_type($template_file)
  {
  ...
  public function etag_expired($cache)
  {
  ...
  public function render($file_path, $template_file)
  {
    $cache = new Cache($file_path, $template_file);
    # set any custom headers
    $this->set_content_type($template_file);
    header('Generator: stacey-v'.static::$version);
  ...
  public function create_page($file_path)
  {
